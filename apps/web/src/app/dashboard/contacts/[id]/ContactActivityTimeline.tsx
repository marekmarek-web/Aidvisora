...
export function ContactActivityTimeline({ contactId }: { contactId: string }) {
  ...
  return (
    <div className="rounded-[var(--wp-radius-lg)] border border-[color:var(--wp-border)] bg-[color:var(--wp-surface)] p-6 shadow-sm">
      <h2 className="font-semibold text-[color:var(--wp-text)] mb-4 text-sm">Aktivita</h2>

      {loading && (
        <p className="text-sm text-[color:var(--wp-text-muted)]">Načítání…</p>
      )}

      {!loading && items.length === 0 && (
        <p className="text-sm text-[color:var(--wp-text-muted)]">Zatím žádná aktivita.</p>
      )}

      {!loading && items.length > 0 && (
        <div className="relative space-y-0">
          <div className="absolute left-[7px] top-2 bottom-2 w-px bg-[color:var(--wp-border)]" />
          {items.map((item) => (
            <div key={item.id} className="relative flex gap-3 py-2">
              <div className="relative z-10 mt-1.5 h-[9px] w-[9px] shrink-0 rounded-full bg-[var(--wp-accent)] ring-2 ring-[color:var(--wp-surface)]" />
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-[color:var(--wp-text-muted)] mt-0.5">
