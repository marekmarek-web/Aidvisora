...
export function CalculatorResultsCard({
  ...
  return (
    <div className="bg-[#0d1f4e] text-white rounded-[20px] shadow-[0_16px_48px_rgba(13,31,78,0.14),0_4px_12px_rgba(13,31,78,0.06)] border border-white/10 p-5 sm:p-6 md:p-7 overflow-hidden relative h-full flex flex-col justify-between">
      ...
      <div>
        ...
        <div className="flex items-baseline gap-2 mb-5 relative z-10">
          ...
          <span className="text-lg sm:text-xl md:text-2xl font-medium text-[color:var(--wp-text-tertiary)]">{unit}</span>
        </div>
        ...
        <div className="space-y-0 relative z-10 bg-[color:var(--wp-surface-card)]/5 rounded-xl p-1 backdrop-blur-sm border border-white/10">
          {rows.map((row, i) => (
            ...
            >
              <span className="text-white/75 text-[13px] sm:text-sm">{row.label}</span>
              <span
                className={
                  row.highlight === "gain"
