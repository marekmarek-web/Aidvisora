import { assistantConversations, assistantMessages, contacts, and, eq, desc, isNotNull, gte } from "db";
...
export async function upsertConversationFromSession(
  ...
  await withTenantContext({ tenantId: session.tenantId, userId: session.userId }, async (tx) => {
...
async function resolveConversationOwner(
  ...
  const [row] = await dbService
    .select({
      tenantId: assistantConversations.tenantId,
...
export async function appendConversationMessage(params: {
  ...
  const owner = await resolveConversationOwner(params.conversationId);
  if (!owner) {
    throw new Error("appendConversationMessage: conversation not found");
  }
...
export async function loadResumableExecutionPlanSnapshot(conversationId: string): Promise<ExecutionPlan | null> {
  const owner = await resolveConversationOwner(conversationId);
  if (!owner) return null;
  const row = await withServiceTenantContext(
    { tenantId: owner.tenantId, userId: owner.userId },
    async (tx) => {
      const [r] = await tx
...
export async function patchAssistantConversationDisplayTitleForUser(
  ...
  return await withTenantContext({ tenantId, userId }, async (tx) => {
    ...
    if (!row) {
      return { ok: false, error: "Konverzace nenalezena." } as const;
    }
...
export async function deleteAssistantConversationForUser(
  ...
  const deletedRows = await withTenantContext({ tenantId, userId }, async (tx) => {
    return await tx
