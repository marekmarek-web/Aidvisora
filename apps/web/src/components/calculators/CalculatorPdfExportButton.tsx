...
export function CalculatorPdfExportButton({
  ...
  return (
    <div className="flex w-full flex-col items-stretch gap-2 md:w-auto md:items-end">
            ...
            try {
              ...
              const msg =
                e instanceof Error && e.message
                  ? e.message
                  : "PDF se nepodařilo vygenerovat. Zkuste to znovu nebo kontaktujte podporu.";
              setError(msg);
            } finally {
              setLoading(false);
            }
      ...
      {error ? (
        <p className="max-w-[min(100%,280px)] text-left text-xs leading-snug text-red-600 md:text-right" role="alert">
          {error}
        </p>
      ) : null}
    </div>
