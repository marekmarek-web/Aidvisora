...
- **Bez vazeb `parent_id` v tenantu:** pokud **žádný** člen nemá vyplněný nadřízeného, **`my_team` vrací jen aktuálního uživatele** — nelze bezpečně určit větev; dřívější fallback „všichni v tenantu“ byl odstraněn (Fáze 7) kvůli úniku rozsahu. V UI se zobrazí banner a upozornění ve struktuře týmu; po doplnění `parent_id` v Nastavení → Tým se rozsah chová očekávaně.
...
Související QA a release poznámky: [`docs/team-overview-release-checklist.md`](./team-overview-release-checklist.md).
